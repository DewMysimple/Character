...
代码字符从代码卡片中心开始坍塌，沿着重力、风力和中心吸引力实时掉落；当字符进入花丛高度后，才经过艺术指导式的状态替换成为蝴蝶。蝴蝶飞向对应花朵并持续盘旋，花园在 8 秒内完成生长。
...
这个工程不使用原始视频作为背景，也不依赖原作者源码。字符、蝴蝶和花朵是三个独立对象，转换通过状态机和透明度交叉淡化完成。字符从代码卡片中部优先缺失，先被吸向卡片底部中央的坍塌点，再获得向下的加速度形成字符流；只有掉落到花丛高度后才开始淡出并生成蝴蝶。重力、风力、中心吸引、阻尼、旋转和扰动负责提供物理感，花朵的茎、叶片和花瓣则按事件逐步生长。控制台中的扇翅频率可以实时调节，默认频率已调低以保持更轻的线稿节奏。
