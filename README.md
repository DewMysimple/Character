...
这是一个 React + Vite + TypeScript 的 Canvas 数字艺术实验，用程序化线稿复刻参考视频前 5 秒的视觉逻辑：

代码字符受重力掉落，经过艺术指导式的状态替换成为蝴蝶，蝴蝶再触发花朵生长。
...
这个工程不使用原始视频作为背景，也不依赖原作者源码。字符、蝴蝶和花朵是三个独立对象，转换通过状态机和透明度交叉淡化完成。重力、风力、中心吸引和阻尼负责提供物理感，花朵的茎、叶片和花瓣则按事件逐步生长。

每次重播使用同一个随机种子，所以调整参数时可以稳定比较差异。
